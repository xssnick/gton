...
| `gton_sync_last_publish_timestamp_seconds` | gauge | none | Unix timestamp of the last published current state. |
...
time() - gton_sync_last_publish_timestamp_seconds
```
...
| `gton_storage_cell_db_cache_bytes` | gauge | `generation`, `role`, `cache` | Cell DB cache size. `cache` is `block` or `file`. |
| `gton_storage_cell_db_cache_requests_total` | counter | `generation`, `role`, `result` | Block cache requests. `result` is `hit` or `miss`. |
| `gton_storage_cell_db_file_cache_tables` | gauge | `generation`, `role` | Number of file cache tables. |
| `gton_storage_cell_db_disk_bytes` | gauge | `generation`, `role`, `shard` | Disk space used by cell DB tables. |
| `gton_storage_cell_db_live_bytes` | gauge | `generation`, `role`, `shard` | Live table bytes. |
| `gton_storage_cell_db_live_tables` | gauge | `generation`, `role`, `shard` | Live table count. |
| `gton_storage_cell_db_read_amp` | gauge | `generation`, `role`, `shard` | Read amplification estimate. |
| `gton_storage_cell_db_l0_files` | gauge | `generation`, `role`, `shard` | Number of L0 files. |
| `gton_storage_cell_db_l0_sublevels` | gauge | `generation`, `role`, `shard` | Number of L0 sublevels. |
| `gton_storage_cell_db_l0_bytes` | gauge | `generation`, `role`, `shard` | Bytes in L0 tables. |
| `gton_storage_cell_db_compaction_debt_bytes` | gauge | `generation`, `role`, `shard` | Estimated compaction debt. |
| `gton_storage_cell_db_compactions_in_progress` | gauge | `generation`, `role`, `shard` | Number of compactions currently running. |
| `gton_storage_cell_db_compaction_in_progress_bytes` | gauge | `generation`, `role`, `shard` | Bytes currently being compacted. |
| `gton_storage_cell_db_memtable_bytes` | gauge | `generation`, `role`, `shard` | Memtable bytes. |
| `gton_storage_cell_db_memtable_count` | gauge | `generation`, `role`, `shard` | Memtable count. |
| `gton_storage_cell_db_table_iters` | gauge | `generation`, `role`, `shard` | Open table iterator count. |
| `gton_storage_cell_db_flushes_total` | counter | `generation`, `role`, `shard` | Pebble flush count. |
| `gton_storage_cell_db_ingests_total` | counter | `generation`, `role`, `shard` | Pebble ingest count. |
| `gton_storage_cell_db_read_cells_total` | counter | `generation`, `role`, `shard` | Cell records successfully read from the cell DB. |
| `gton_storage_cell_db_written_cells_total` | counter | `generation`, `role`, `shard` | Cell records successfully written to the cell DB. |
...
```promql
max(gton_storage_cell_db_compaction_debt_bytes{shard="total"}) by (generation, role)
max(gton_storage_cell_db_l0_files{shard="total"}) by (generation, role)
sum(rate(gton_storage_cell_db_cache_requests_total[5m])) by (result)
sum(rate(gton_storage_cell_db_read_cells_total[1m])) by (generation, role, shard)
sum(rate(gton_storage_cell_db_written_cells_total[1m])) by (generation, role, shard)
```
