...
func TestHandleSendMessageForwardsExternalBOC(t *testing.T) {
	...
	store := &fakeStore{
		...
		blocks: map[string][]byte{
			storage.BlockKey(block): blockRoot.ToBOC(),
		},
...
func TestLiveStoreServesPendingBlockBeforeStorageFlush(t *testing.T) {
	...
	payload := root.ToBOCWithFlags(false)
...
func TestLiveStorePublishesCurrentOnlyAfterShardDataFlushed(t *testing.T) {
	...
	shardData := shardRoot.ToBOCWithFlags(false)
...
func TestLiveStoreStateMethodsUseCurrentStateBeforeStorage(t *testing.T) {
	...
	blockData := blockRoot.ToBOCWithFlags(false)
...
func TestLiveStoreBlockRootAcceptsTrustedStoredRootWithNonCanonicalBOC(t *testing.T) {
...
func TestLookupBlockBySeqnoReturnsBlockHeader(t *testing.T) {
	...
	store := &fakeStore{
		blocks: map[string][]byte{
			storage.BlockKey(id): root.ToBOC(),
		},
		seqLookup: id,
	}
	srv := testServer(store)
...
func TestAccountStateReturnsAccountCellAndProofRoots(t *testing.T) {
	...
	store := &fakeStore{
		blocks: map[string][]byte{
			storage.BlockKey(id): blockRoot.ToBOC(),
		},
		blockStates: map[string]*storage.BlockState{
			storage.BlockKey(id): {
				Block:         id,
...
func TestAccountStateWithMasterReferenceResolvesBasechainShard(t *testing.T) {
	...
	store := &fakeStore{
		blocks: map[string][]byte{
			storage.BlockKey(masterBlock): masterRoot.ToBOC(),
			storage.BlockKey(shardBlock):  shardRoot.ToBOC(),
		},
		blockStates: map[string]*storage.BlockState{
...
func TestRunSmcMethodExecutesGetMethodAndReturnsProofs(t *testing.T) {
	...
	c7Value, err := tlb.ParseStackValue(result.InitC7.BeginParse())
	if err != nil {
	...
	if err = stack.LoadFromCell(result.Result.BeginParse()); err != nil {
...
func TestRunMethodConfigBuildsCppC7Extras(t *testing.T) {
	...
	precompiled, ok := cfg.Precompiled.(*big.Int)
	if !ok || precompiled.Uint64() != 777 {
...
func TestHandleConfigParamsPreviousKeyBlockMode(t *testing.T) {
	...
	store := &fakeStore{
		blocks: map[string][]byte{
			storage.BlockKey(keyID):   keyRoot.ToBOC(),
			storage.BlockKey(blockID): blockRoot.ToBOC(),
		},
		seqLookup: keyID,
	}
	srv := testServer(store)
...
func TestHandleGetShardInfoReturnsDescriptorFromMasterState(t *testing.T) {
	...
	store := &fakeStore{
		blocks: map[string][]byte{
			storage.BlockKey(master): blockRoot.ToBOC(),
		},
		blockStates: map[string]*storage.BlockState{
...
func TestHandleOutMsgQueueSizesUsesCurrentMasterAndShardStates(t *testing.T) {
	...
	store := &fakeStore{
		...
		blocks: map[string][]byte{
			storage.BlockKey(master): masterRoot.ToBOC(),
			storage.BlockKey(shard):  shardRoot.ToBOC(),
		},
		blockStates: map[string]*storage.BlockState{
...
func keyBlockConfigProofParamCell(t *testing.T, keyRoot *cell.Cell, proof []byte, param int32) *cell.Cell {
	...
	var block tlb.Block
	if err := tlb.LoadFromCell(&block, body.BeginParse()); err != nil {
...
func (s *fakeStore) LoadStateCellTree(_ context.Context, block ton.BlockIDExt, rootHash []byte) (*cell.Cell, uint64, error) {
	...
	if ok && state.Cell != nil {
		...
		if len(rootHash) == 0 || bytes.Equal(hash[:], rootHash) {
			return state.Cell, state.CellsCount, nil
		}
	}
