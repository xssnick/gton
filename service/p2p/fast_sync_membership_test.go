...
var (
	fastSyncMembershipCountSink fastSyncMembershipCounts
...
func TestFastSyncMembershipCachedOmittedCertificate(t *testing.T) {
	...
	root.certificates = newFastSyncMemberCertificateCache()
		...
		t.Run(test.name, func(t *testing.T) {
			...
			membership := newFastSyncMembership(
				fastSyncMembershipTestRoster(
					[]PeerID{firstIssuer.id, incomingIssuer.id},
					nil,
				),
				0,
			)
...
func TestFastSyncMembershipRetainsRootState(t *testing.T) {
	...
	membership := newFastSyncMembership(
		fastSyncMembershipTestRoster(
			[]PeerID{retainedIssuer.id, removedIssuer.id},
			[]PeerID{retainedPermanent},
		),
		0x10,
	)
	if err := membership.EnrollPermanentNode(
		retainedPermanent,
		0x20,
	); err != nil {
	...
	membership.UpdateRoster(
		fastSyncMembershipTestRoster(
			[]PeerID{retainedIssuer.id, addedIssuer.id},
			[]PeerID{
				retainedPermanent,
				addedPermanent,
				transitionNode,
...
func TestFastSyncMembershipPermanentDemotion(t *testing.T) {
	...
	t.Run("retains_received_certificate", func(t *testing.T) {
		membership := newFastSyncMembership(roster, 0x10)
		if err := membership.EnrollPermanentNode(node, 0x20); err != nil {
	...
	t.Run("roster_refresh_clears_permanent_certificate", func(t *testing.T) {
		membership := newFastSyncMembership(roster, 0)
		if err := membership.AuthorizeMember(
			node,
			certificate,
			now,
...
func TestFastSyncMembershipRateAndLRU(t *testing.T) {
	...
	var firstKey fastSyncMemberCertificateCacheKey
...
func TestFastSyncMembershipPermanentOmittedCertificate(t *testing.T) {
	...
	membership := newFastSyncMembership(
		fastSyncMembershipTestRoster(nil, []PeerID{permanent}),
		0x11,
	)
	...
	if err := membership.EnrollPermanentNode(
		unknown,
		0x22,
	); !errors.Is(err, ErrFastSyncNotFound) {
...
func TestFastSyncMembershipCountsAllocations(t *testing.T) {
	membership := newFastSyncMembership(
		fastSyncMembershipTestRoster(
			nil,
			[]PeerID{fastSyncTestPeerID(0xb1)},
		),
		0,
	)
...
func fastSyncMembershipTestCertificateCacheKey(
	...
	node PeerID,
	certificate overlay.MemberCertificate,
) fastSyncMemberCertificateCacheKey {
	t.Helper()
