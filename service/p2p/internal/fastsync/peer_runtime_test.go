...
type testPeerRuntime struct {
	*PeerRuntime
}

func newTestPeerRuntime(
	...
	now time.Time,
) (*testPeerRuntime, error) {
	runtime, err := NewPeerRuntime(
		privateKey,
		overlayID,
		localFlags,
		certificate,
	...
	return &testPeerRuntime{PeerRuntime: runtime}, nil
}

func (r *testPeerRuntime) UpdateRoster(
	roster testRoster,
	permanentFlags uint32,
	now time.Time,
) {
	r.PeerRuntime.updateRoster(
		roster.roots,
		roster.permanent,
		permanentFlags,
		now,
...
func TestFastSyncPeerRuntimeLocalNode(t *testing.T) {
	...
	t.Run("permanent", func(t *testing.T) {
		...
		roster := peerRuntimeTestRoster(
			publicKey,
			testValidator{ADNLID: localID},
		)
		membership := newTestMembership(roster, 0x01)

		runtime, err := newTestPeerRuntime(
			privateKey,
			overlayID,
			0x80,
...
func TestFastSyncPeerRuntimeRandomPeers(t *testing.T) {
	...
	roster := peerRuntimeTestRoster(
		localPublic,
		testValidator{ADNLID: localID},
		testValidator{ADNLID: permanentID},
	)
	membership := newTestMembership(roster, 1)
	runtime, err := newTestPeerRuntime(
		localKey,
		overlayID,
		0,
	...
	if len(response.Nodes) != RandomPeerResultLimit {
		t.Fatalf(
	...
	seen := make(map[ID]struct{}, len(response.Nodes))
...
func TestFastSyncRandomPeersMaxAnswerMatchesFourFullNodes(t *testing.T) {
	...
	const maximumWireSize = 8 + RandomPeerResultLimit*264
	...
	nodes := make([]overlay.NodeV2, RandomPeerResultLimit)
	for i := range nodes {
		nodes[i] = node
	}
...
func TestFastSyncPeerRuntimeRosterUpdate(t *testing.T) {
	...
	if _, err = peerRuntimeStoredNode(runtime, clientID); !errors.Is(err, ErrNotFound) {
		t.Fatalf("removed peer lookup error = %v, want %v", err, ErrNotFound)
	}
}
...
func BenchmarkFastSyncPeerDescriptorLimiterFull(b *testing.B) {
	...
	b.ResetTimer()
	for range b.N {
		if runtime.peerDescriptorAvailableLocked(now) {
...
func peerRuntimeTestRootRuntime(
	...
	overlayID OverlayID,
	now time.Time,
) (*testPeerRuntime, ed25519.PrivateKey) {
	t.Helper()
...
func peerRuntimeTestRoster(
	...
	copy(rootPublicKey[:], rootPublic)
	rootID := ValidatorID[ID](rootPublicKey)

	permanent := make([]ID, 0, len(validators))
	...
	slices.SortFunc(permanent, func(left, right ID) int {
		return bytes.Compare(left[:], right[:])
	})
	permanent = slices.Compact(permanent)
...
func peerRuntimeTestPeerID(publicKey ed25519.PublicKey) ID {
	var value [ed25519.PublicKeySize]byte
	copy(value[:], publicKey)
	return ValidatorID[ID](value)
}

func peerRuntimeTestOverlayID(marker byte) OverlayID {
	var id OverlayID
	for index := range id {
...
func peerRuntimeStoredNode(
	...
	if peer == nil {
		return overlay.NodeV2{}, ErrNotFound
	}
