...
func TestWaitSyncDiskSpaceReturnsWhenEnoughSpace(t *testing.T) {
	...
	if err := svc.waitSyncDiskSpace(context.Background(), "next_block"); err != nil {
...
func TestWaitSyncDiskSpaceRetriesAndWakesMaintenance(t *testing.T) {
	...
	svc := &Service{
		...
		syncDiskSpacePath:       "/db",
		minSyncDiskFreeBytes:    100,
		syncDiskSpaceRetryDelay: time.Millisecond,
		syncDiskSpaceProbe: func(path string) (syncDiskSpaceStatus, error) {
			calls++
			if calls == 1 {
	...
	if err := svc.waitSyncDiskSpace(context.Background(), "archive_catchup"); err != nil {
		t.Fatalf("wait disk space: %v", err)
	}
	if calls != 2 {
...
func TestWaitSyncDiskSpaceReturnsContextErrorWhileWaiting(t *testing.T) {
	...
	svc := &Service{
		syncDiskSpacePath:       "/db",
		minSyncDiskFreeBytes:    100,
		syncDiskSpaceRetryDelay: time.Hour,
		syncDiskSpaceProbe: func(path string) (syncDiskSpaceStatus, error) {
			return syncDiskSpaceStatus{Path: path, AvailableBytes: 99}, nil
		},
	}

	if err := svc.waitSyncDiskSpace(ctx, "next_block"); err != context.Canceled {
